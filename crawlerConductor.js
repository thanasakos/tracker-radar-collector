...
async function crawlAndSaveData(urlString, dataCollectors, log, filterOutFirstParty, dataCallback, emulateMobile, proxyHost, antiBotDetection, executablePath, maxLoadTimeMs, extraExecutionTimeMs, collectorFlags, outputPath) {
    ...
    const data = await crawl(url, {
        ...
        collectors: dataCollectors.map(collector => (collector.constructor.name === 'RequestContentCollector' ? new collector.constructor({contentOutputFolder: outputPath}) : new collector.constructor())),
        filterOutFirstParty,
        emulateMobile,
        proxyHost,
