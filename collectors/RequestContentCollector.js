...
class RequestContentCollector extends BaseCollector {
    ...
    constructor(additionalOptions = {saveHeaders: DEFAULT_SAVE_HEADERS}) {
        ...
        if (additionalOptions.contentOutputFolder !== undefined) {
            this._contentOutputFolder = additionalOptions.contentOutputFolder + "content/";
    ...
    async handleFailedRequest(data, cdp) {
        ...
        if (this._saveResponseHash) {
            request.responseBodyHash = await this.getResponseBodyAndSaveHash(data.requestId, cdp, this._contentOutputFolder, type);
        }
    }
...
module.exports = RequestContentCollector;
...
/**
 * @typedef {'Document'|'Stylesheet'|'Font'|'Script'|'TextTrack'|'XHR'|'Fetch'|'EventSource'|'WebSocket'|'Manifest'|'SignedExchange'|'Ping'|'CSPViolationReport'|'Other'} ResourceType
 */
